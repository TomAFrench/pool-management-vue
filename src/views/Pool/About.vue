<template>
  <UiTable class="p-4">
    <div v-if="bPool.config.about" class="mb-3">
      ...
      <h5 v-text="bPool.config.about" class="mb-2" style="max-width: 580px;" />
    ...
    <div v-if="bPool.isCrp()" class="mb-3">
      ...
      <template v-if="Object.keys(rights).length > 0">
        <div v-for="(right, key) in rights" :key="key">
          <h5 v-text="poolRights[key]" class="mb-1" />
        </div>
      </template>
      <h5 v-else v-text="$t('none')" />
    </div>
    ...
    <div v-if="bPool.isCrp() && bPool.metadata.crpController" class="mb-3">
      ...
      <h5>
        <a :href="_etherscanLink(bPool.metadata.crpController)" target="_blank">
    ...
    <div class="mb-3">
      ...
      <h5>
        <a :href="_etherscanLink(bPool.metadata.tx, 'tx')" target="_blank">
    ...
    <template v-if="bPool.metadata.finalized">
      <div class="mb-3">
        ...
        <h5>
          <a :href="_etherscanLink(bPool.address, 'token')" target="_blank">
      ...
      <div class="mb-3">
        ...
        <h5 v-text="_num(bPool.metadata.totalShares)" />
      </div>
    </template>
    ...
    <div class="mb-3">
      ...
      <h5 v-text="_num(bPool.metadata.swapFee, 'percent')" />
    </div>
    <div class="mb-3">
      <div v-text="$t('totalSwapVolume')" class="mb-2" />
      <h5 v-text="_num(bPool.metadata.totalSwapVolume, 'usd')" />
    </div>
    <div class="mb-3">
      <div v-text="$t('totalSwapFee')" class="mb-2" />
      <h5 v-text="_num(bPool.metadata.totalSwapFee, 'usd')" />
    </div>
    <div class="mb-3">
